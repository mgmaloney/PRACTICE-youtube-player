...
const renderToDom = (divId, htmlToRender) => {
  let targetDiv = document.querySelector(`${divId}`);
  targetDiv.innerHTML = htmlToRender;
};
...
const eventListeners = () => {
  ...
  document.querySelector("#filterContainer").addEventListener("click", (e) => {
    ...
    if (e.target.id === "favorite") {
      const favoriteArr = data.filter((item) => item.favorite === true);
      console.log(favoriteArr);
      cardsOnDom(favoriteArr);
    } else if (e.target.id === "clear") {
      cardsOnDom(data);
    } else {
      const filteredArr = data.filter((item) => item.category === e.target.id);
      console.log(filteredArr);
      cardsOnDom(filteredArr);
    }
  ...
  document.querySelector("#cardContainer").addEventListener("click", (e) => {
    ...
    if (e.target.id) {
      ...
      if (e.target.id.includes("delete")) {
        ...
        let indexOfItem = data.findIndex(
